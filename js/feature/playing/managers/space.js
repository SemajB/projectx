        ...
        update(event) {
          ...
          for (let i = active.length - 1; i > 0; i--) {
            ...
            hit: for (let j = i - 1; j > -1; j--) {
              const
                bodyB = active[j];
                if(bodyA.type === 'hitzone-exit' && bodyB.type === 'hitzone-exit') continue;
              
              const
                distanceAttributes = getDistanceAttributes(bodyA, bodyB),
                hitResult = doRadiiHitTest(distanceAttributes.distance, bodyA, bodyB);
              if (hitResult.isHit) {
                const player = bodyA.type === 'player' ? bodyA : bodyB;
                const hitzone = bodyA.type === 'hitzone-exit' ? bodyA : bodyB;
                console.log('hit-exit');
                messenger.dispatch({ type: 'EXIT', exit: hitzone.exit, source: 'space'  });
